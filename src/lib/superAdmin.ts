...
/**
 * 진행 건너뛰기 모드 — 순서를 무시하고 앱을 시험한다.
 *
 * **이름이 옛것이다.** 파일도 함수도 `superAdmin`으로 시작하지만, 이 모드를
 * 켜는 것은 **일반관리자부터**다(`isAdminUser()`). 등급을 둘로 가르기 전에는
 * 관리자가 하나뿐이라 그 이름이 맞았다 — 지금 슈퍼관리자가 뜻하는 것은
 * 참여자 데이터를 보는 쪽이고(`isSuperAdminUser()`), 이 모드와 무관하다.
 * 부르는 자리가 열세 곳이라 이름은 두었고, 화면에 보이는 글씨만 고쳤다.
 *
