...
export type CueId =
  | 'B0_TAPE' | 'B0_CALL'
  | 'B1_A' | 'B1_OK' | 'B1_S' | 'B1_B' | 'B1_YES' | 'B1_NO' | 'B1_WALK'
  | 'B2_A' | 'B2_B'
  | 'B3_A' | 'B3_B'
...
export interface SubtitleLine {
   ...
   * 이 줄을 다 말하고 **쉬는 시간(초)**.
   *
   * 음원에 무음을 굽지 않는다. 굽는 방식은 뜸을 고치려면 그 줄을 다시
   * 구워야 하고, 길이가 바뀌면 자막 시각을 통째로 다시 재야 한다.
   * 여기서는 엔진이 재생을 잠깐 멈췄다 잇는다 — 값만 바꾸면 끝이다.
   *
   * 대본(`SCRIPT.md`)에는 적지 않는다. 뜸은 연출이지 대사가 아니라서,
   * 글에 `(2s)` 같은 표기가 섞이면 자막에 그대로 나오고 TTS도 읽는다.
   */
  pauseSec?: number
}
...
export const CUES: Record<CueId, Cue> = {
  ...
  B0_TAPE: {
    ...
    subtitleLines: [
      ...
      { text: '강진욱. 아빠가 지었다.' },
      { text: '이런 거 왜 하고있냐 싶다마는.<br>아빠가 원래 말주변이 없다.<br>그러니까 여기에 녹음하는 거다.' },
      { text: '너 기다리면서… 뭐, 소원이랄 것까진 없고.<br>다섯 개를 생각해봤다.' },
      { text: '하나, 엄마랑 어떻게 만났는지 들려주기.<br>' },
      { text: '둘. 목욕탕 가서 서로 등 밀어주기.' },
      { text: '셋, 예쁜 사진 찍어주기.' },
      { text: '넷, 좋아하는 음악 같이 듣기.' },
      { text: '다섯… 가족오락관 같이 나가기.' },
      { text: '네가 크면 이 테이프를 같이 듣자. 몇 개 지켜졌나 세어보고.' },
      { text: '뭐, 그런 약속이다.' },
      { text: '얼른 만나고 싶구나.' },
    ],
  ...
  B0_CALL: {
    ...
    subtitleLines: [
      ...
      { text: "여자 목소리라 놀라셨나요? 테이프에서 '강진욱'이라는 이름, 들으셨죠?" },
      { text: '아빠는 아들이 태어날 줄 아셨대요.<br> 낳고 보니 딸이었죠.' },
      { text: '방법이… 이것밖에 생각이 안 났어요.<br>전단지를 붙일까도 했는데, 그건 아닌 것 같았고.' },
      { text: '아빠 목소리를 먼저 들려드리고 싶었어요.' },
      { text: '그 목소리를 끝까지 듣고도 그냥 지나가시는 분이라면,<br>어차피 부탁드릴 수 없는 거니까요.' },
      { text: '…끝까지 들어주셨네요.' },
      { text: '아버지가 얼마 전 넘어져서 다리에 깁스를 하셨어요.<br>그리고 요즘 자꾸 깜빡깜빡하세요.' },
      { text: '근데 신기하죠, 제가 태어나던 시기는 아직도 또렷하게 기억하세요.' },
      { text: '저도 사실… 그 테이프, 소원 부분까지 들었어요.<br>아빠 목소리가 너무 젊어서 신기했죠.' },
      { text: '소원들이 전부 이 동네 어딘가에 있어요. 걸어서 십 분 거리인데…<br>저도 며칠 전에 이 집 계단에서 발목이 부러졌어요.<br>아빠도 저도, 지금 한 발짝을 못 나가요.' },
      { text: '그래서… 같이 가주시겠어요?<br>제가 전화 통화하며 함께 걸을게요.' },
      { text: '소원은 제가 이룰 거예요.<br>그냥, 옆에서 같이 걸어주시면 돼요.' },
      { text: '첫 번째 장소는 거기서 3분이에요.<br>우물이 있던 자리라고, 아빠한테 옛날에 들은 적이 있어요.<br>지금은 카페가 됐대요.' },
      { text: '저랑 같이 걸어주실래요?' },
    ],
    sfx: { in: 'call_ring' },
    next: null,
  ...
  */
  ...
  B1_S: {
    ...
    subtitleLines: [
      ...
      { text: '풍선초야. 이게 왜 여태 자라는 줄 알아?<br>네 아버지가 심은 거야.' },
      { text: "저 우물가에서, 네 아버지가 이 풍선초로 하트를 딱 만들어서<br>네 엄마한테 그러더라.<br>'이게… 내 마음이오.'" },
      { text: '아이고, 그 무뚝뚝한 사람이.<br>온 동네가 다 웃었지.' },
    ],
    next: null,
    ui: ['show_mission:M1b'],
  },
  ...
  */
  B1_B: {
    ...
    subtitleLines: [
      ...
      // 말을 놓자고 꺼내기 전에 한 박자 — 묻기 어려운 말이라 뜸이 필요하다
      { text: '첫 번째 아버지가 들려주고 싶었던 이야기를 들었네요.<br>엄마와의 러브스토리를 방금 들었으니까.', pauseSec: 2 },
      { text: '그런데...<br>저기… 말 편하게 해도 될까요?<br>왠지 오래 알던 사이 같아서요.' },
    ],
    next: null,
  ...
  */
  B1_NO: {
    ...
    subtitleLines: [
      ...
      { text: '첫 번째, 소원을 방금 같이 이뤘어. 진짜 고마워.' },
      { text: '이제 가게에서 나와서, 오던 길로 쭉 걸어볼래?' },
    ],
    next: 'B1_WALK',
    ui: ['speech_mode:casual'],
  ...
  /*
    ...
    말 놓기 두 갈래(B1_YES·B1_NO) 어느 쪽으로 갔든 여기로 모인다. 두 분기에
    같은 줄을 복제하지 않으려고 큐를 따로 세웠다 — 복제하면 한쪽만 고치는
    일이 반드시 생긴다.

    걷는 동안 듣는 대목이라 **미션이 걸리지 않는다.** 도착은 여전히 미야상회
    QR이 연다(D4의 다섯 거점은 그대로다).
  */
  B1_WALK: {
    id: 'B1_WALK',
    track: 1,
    ...
    subtitleLines: [
      ...
      { text: '예전엔 어두운 동네였는데 많이 바뀌었더라' },
      { text: '여길 이제는 봉리단길이라고 부른대, 카페도 많아졌고..<br>내가 떠날 땐 그냥 봉황동 이었는데.' },
      { text: '거기서 조금만 더 가면, 왼쪽에 서부탕이 있었어.<br>지금은 카페가 됐다더라.' },
      { text: '아빠는 매주 갔대. 나만 못 데려가고.' },
      { text: '그 앞을 지나서 조금 더 가면 미야상회라고,<br>조그만 점빵이 하나 있어.' },
    ],
    next: null,
  },
  ...
  B2_A: {
    ...
    subtitleLines: [
      ...
      { text: '그래서 아빠는 목욕탕에 못 가는 대신,<br>매주 일요일마다 그 가게에서 바나나우유를 사 왔어.' },
      { text: "나 그게 그냥 바나나우유인 줄 알았어.<br>그게 '목욕탕 대신'이었다는 걸, 이번에 내려와서 이제야 알았네." },
      { text: '…그러니까 네가 나 대신 바나나우유를 찾아 마셔줘.<br>그리고 무슨 맛인지 알려줘.' },
    ],
    next: null,
    ui: ['show_mission:M2'],
  },
  ...
  B2_B: {
    ...
    subtitleLines: [
      ...
      // 울음을 삼키는 자리 — "괜찮아, 나 안 울어"가 바로 붙으면 삼킬 틈이 없다
      { text: '아빠가 나한테 관심이 없는 줄 알았어.<br>나는 서운해할 줄만 알았지, 19년 동안.', pauseSec: 2 },
      { text: '괜찮아, 나 안 울어.<br>두 번째도 이뤘다, 이제 우리. 가자.' },
      { text: '그 골목 끝까지 쭉 가면 오래된 한옥이 하나 있어.' },
  ...
  B3_A: {
    ...
    subtitleLines: [
      ...
      { text: "세 번째 소원이 사진 찍어주기잖아.<br>아빠가 꽃 활짝 폈을 때 '여기서 사진 찍자' 그랬는데…" },
      { text: '나 그때 사춘기라고 싫다고 했어.<br>그러고 그 이후론 못 찍었지.<br>내가 큰 이후론 같이 찍은 사진도 없어.' },
      { text: '휴대폰 들어서 담장 한번 비춰볼래?<br>요즘엔 그런 게 된다더라.' },
    ],
    next: null,
    ui: ['show_mission:M3'],
  },
  ...
  B3_B: {
    ...
    subtitleLines: [
      ...
      { text: "세 개째다. 네 번째 소원이 좀 문제야.<br>'좋아하는 음악 같이 듣기'인데…<br>아빠가 무슨 음악을 좋아했는지, 나 모르겠어." },
      { text: '밤마다 라디오를 들으셨던 건 기억나는데.' },
      { text: '조금 찾기 어려울 텐데, 저 앞에 오른쪽으로 좁은 골목 있지?<br>거기로 들어가면 카페 탱자가 있어.' },
  ...
  B4_A: {
    ...
    durationSec: 10,
    subtitleLines: [
      { text: '들어왔어? 창가 자리 있으면 앉아.<br>볼륨은 조금만 키우고.' },
  ...
  B4_RADIO: {
    ...
    durationSec: 43,
    subtitleLines: [
      // 사연을 펼쳐 읽기 전의 뜸 — DJ가 종이를 집는 시간이다
      { text: '다음은... 오늘도 사연을 보내주셨네요.', pauseSec: 2 },
      { text: '…딸아이가 여섯 살입니다.<br>아들을 바랐는데, 딸이 나왔네요.<br> 그래도 세상에서 제일 예쁜 아이입니다.' },
      { text: "근데 제가 말재주가 없어서,<br>'아빠가 너를 사랑한다' 이 한마디를 제대로 못 해봤습니다." },
      { text: '우리 딸, 영원히 지금처럼 웃는 소녀로 남았으면 합니다.' },
      { text: '소영아.<br>… 아빠는, 네가 웃을 때가 제일 행복하다.' },
      { text: "봉황동 강민수 씨의 사연이었습니다.<br>라디오 사연으로 전해 주셨는데.. 언젠가는 직접 전해주시면 어떨까요<br><br>신청곡은 이문세 '소녀'입니다. 함께 듣겠습니다." },
    ],
    sfx: { in: 'tape_hiss' },
    next: 'B4_B',
  },
  ...
  B4_B: {
    ...
    subtitleLines: [
      ...
      { text: '…근데 너도, 지금 떠오르는 사람 있지?' },
      { text: '그 사람한테 못 한 말… 나한테 해볼래?<br>내가 대신 들어줄게.' },
      { text: '아빠도 그랬잖아.<br>얼굴 보고 못 하는 말은, 이렇게라도 전할 수 있는 거야.' },
    ],
    next: null,
  ...
  B5_A: {
    ...
    durationSec: 9,
    subtitleLines: [
      { text: '가고있어? 주변에 뭐 붙어 있는 거 없어?<br>포스터 같은 거.' },
  ...
  B5_T1: {
    ...
    durationSec: 14,
    subtitleLines: [
      { text: '아, 그거. 아빠 집 올 때 얼핏 본 것 같아.<br>동네에서 축제 한다고.' },
      { text: '…근데 잠깐, 가족오락관?' },
      { text: '가게 안에 들어가서,<br>혹시 아는 게 있으신지 여쭤봐 줄래?' },
    ],
    next: null,
    ui: ['show_ask_button'],
  },
  ...
  B5_T2: {
    ...
    durationSec: 21,
    subtitleLines: [
      { text: '혹시… 강민수 어르신 일로 오셨어요?<br>어르신이 얼마 전에 오셨어요.' },
      { text: '축제 가족오락관에 참가 신청을 하시더라고요.<br>우리 딸이 올 거라고.' },
      { text: '오면 꼭 같이 나가게 해달라고.<br>신청서에 두 사람 이름을 적어놓고 가셨어요.<br>강민수, 강소영.' },
    ],
    next: 'B5_T3',
  },
  ...
  B5_T3: {
    ...
    subtitleLines: [
      ...
      { text: '…그 테이프, 뒤집어서 B면 틀어줄래?<br>아빠가 여기까지 준비한 사람이면…<br>뭔가 있을지도 몰라.' },
    ],
    next: null,
  ...
  B5_LETTER: {
    ...
    subtitleLines: [
      ...
      { text: '미안하다.<br>그리고 사랑한다. <br>사랑한다는 말, 이게 뭐라고 너무 늦었구나.' },
      { text: '다음 주에, 가족오락관 같이 나가자.<br>마지막 소원은 아빠가 너랑 함께 이루고 싶다.' },
      { text: '…고맙다, 소영아.' },
    ],
    sfx: { in: 'tape_hiss', out: 'tape_stop' },
    next: 'B5_F',
  ...
  B5_F: {
    ...
    subtitleLines: [
      ...
      { text: '다음 주엔 나는 목발 짚고, 아빠는 휠체어 타고라도 같이 나갈 거야.<br>둘 다 절뚝거리면서. 하하하<br>마지막 소원은 우리가 직접 해볼게.' },
      { text: '오늘 처음 만났는데… 정말 고마워.<br>넌, 이제 내 친구야. 진심으로.' },
    ],
    next: 'B6_0',
    /*
      4+1: 5번째 소원은 '다음 주' 리본으로 유보.

      쿠폰은 여기서 주지 않는다. **거점마다 그 미션을 끝낸 자리에서** 한 장씩
      나온다(cueEngine의 dispatchAction이 TRACK_MISSIONS의 reward를 읽는다).

      한동안 이 자리에 "빙고 첫 줄에서 다섯 장을 한꺼번에 준다"고 적혀
      있었다. 그 일괄 지급은 보상 자리가 둘로 갈려 흐려진다는 이유로
      없앴는데, 주석만 남아서 쿠폰이 어디선가 나오는 줄로 읽혔다 —
      실제로는 어디서도 나오지 않았다.
    */
    ui: [
      'track_check:5',
  ...
  B6_0: {
    ...
    durationSec: 23,
    subtitleLines: [
      { text: '…자, 무거운 얘긴 여기까지!<br>눈물 쏙 뺐으니까 이제 기분 전환하자.' },
      { text: '사실 나, 이번에 내려오기 전까진 이 동네가 싫었거든.<br>근데 너랑 통화하면서 걸으니까 다시 좋아졌어.' },
      { text: '이제 내가 자란 동네, 구경시켜줄게.<br>천천히, 마음 가는 대로 둘러봐 줘.' },
  ...
  B6_X_BUNSIK: {
    ...
    durationSec: 7,
    subtitleLines: [
      { text: '여기 떡볶이 진짜 맛있어. 나 학교 끝나면 매일 왔어.<br>아직도 하시네, 신기하다.' },
    ],
    next: null,
  },
  ...
  B6_X_BYEOKHWA: {
    ...
    durationSec: 6,
    subtitleLines: [
      { text: '이 골목에서 나 자전거 처음 배웠어.<br>여기서 넘어져서 무릎 다 까졌지.' },
    ],
    next: null,
  },
  ...
  B6_X_BONGHWANGDAE: {
    ...
    durationSec: 5,
    subtitleLines: [
      { text: '여기 올라가면 동네가 다 보여.<br>아빠가 소풍 데려와서 김밥 먹던 데야.' },
    ],
    next: null,
  },
  ...
  B7_0: {
    ...
    subtitleLines: [
      { text: '오늘 하루, 정말 고마웠어.' },
      { text: '그 테이프… A면은 아빠의 서른여덟 해 전이고,<br>B면은 아빠의 지금이야.<br>그 사이를 오늘 너랑 같이 걸었네.' },
      { text: '이제 이 골목은,<br>나 혼자만의 기억이 아니라 우리 둘의 이야기야.' },
    ],
    next: 'B7_1',
  },
