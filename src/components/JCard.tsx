 ...
 *
 * '기억의 조각'을 대체하는 수집 구조다. 줄은 따로 저장하지 않고 트랙
 * 완료(tracksCompleted)에서 파생한다 — 조각은 늘 트랙 완료와 같은 큐에서
 * 지급돼 두 상태가 어긋날 길이 없었고, 그렇다면 상태는 하나면 된다.
 *
 * 다섯째 줄은 영원히 채워지지 않는다. 잠금 아이콘이 아니라 '아버지의 몫'
 * 이라는 상태 문구다 — 미완이 실패가 아니라 이야기의 결말이기 때문이다
 * (B5_F "마지막 하나는 남겨둘게요"). B면 편지를 들은 뒤에는 피날레의
 * '다음 주' 배지와 같은 문구로 바뀐다.
 *
...
/** B면 편지를 들은 뒤 — 피날레의 '다음 주' 배지와 같은 결 */
const FIFTH_WISH_NEXT = '다음 주, 아빠와 소영'
  ...
  */
  ...
  return (
    <div className="rounded-2xl border border-line bg-paper p-5 shadow-sm">
      ...
      <p className="mt-0.5 font-pen text-[15px] text-ink-60">
        소영에게 — 1988. 9. 17.
      </p>
      ...
      <ol className="mt-3">
        {TRACK_STATIONS.map((station) => {
          ...
          return (
            <li key={station.id} className="flex items-baseline gap-2 py-1">
              <span className="font-mono-retro text-[10px] text-ink-60">
                A{station.track}
              </span>
              <span className="min-w-0 flex-1">
                <span
                  className={`block font-pen text-[19px] leading-snug ${
              ...
              <span
                ...
                style={animate ? { animation: 'fadeIn 0.8s ease-in-out' } : undefined}
              >
                {fillable ? (
                  filled ? (
                    '🖋'
                  ...
                  <span className="font-mono-retro text-[9.5px] tracking-wider text-ink-60">
                    {letterHeard ? FIFTH_WISH_NEXT : FIFTH_WISH_RESERVED}
                  </span>
                )}
              </span>
            </li>
      ...
      {letterHeard && (
        <div
          className="mt-3 border-t border-dashed border-line pt-2.5"
        ...
        >
          <div className="flex items-baseline gap-2">
            ...
            <span className="font-pen text-[19px] text-ink">아버지의 편지</span>
            <span className="ml-auto text-[13px]" aria-hidden>
              📼
            </span>
          </div>
        </div>
